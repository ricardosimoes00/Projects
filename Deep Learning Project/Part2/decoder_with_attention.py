...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        e = self.encoder_att(encoder_out)
        d = self.decoder_att(decoder_hidden)
        d = d[:,None,:]
        d = d.expand(-1,e.size(dim=1) ,-1)
        similarity = e + d
        similarity = self.relu(similarity)
        attention = self.full_att(similarity)
        p_attention = self.softmax(attention)
        attention_weighted_encoding = torch.bmm(encoder_out.permute(0, 2, 1),p_attention)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, word, decoder_hidden_state, decoder_cell_state, encoder_out):
        ...
        attention_w = torch.squeeze(self.attention(encoder_out, decoder_hidden_state), dim=2)
        decoder_hidden_state, decoder_cell_state = self.decode_step(torch.cat((embedding, attention_w), 1), (decoder_hidden_state, decoder_cell_state))
