...
test6_forecast <- forecast(test6, level=c(95), h=5)

x <- window(Localidade_TS, 365, c(366,24))
y <- test6_forecast$mean
z <- test6_forecast$upper
w <- test6_forecast$lower

ts.plot(x, y, z, w,  gpars = list(col = c("black", "brown1", "brown", "brown")), main = "Time series forecasting plot", ylab = "Values")

round(cbind(as.matrix(test6_forecast$mean),as.matrix(test6_forecast$lower),as.matrix(test6_forecast$upper)),2)
