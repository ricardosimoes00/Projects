...
setwd("C:/Users/Fernando/Desktop/ST/Sobreiras-Porto")
data <- read_excel("C:/Users/Fernando/Desktop/ST/QualidadeARO3.xlsx")
Localidade <- data$`Sobreiras-Porto`
...
#Residuals check
checkresiduals(test1)
ggarrange(
  plotseriesfunc(test1$residuals,"Residuals Series Plot"),
  ggarrange(plotacffunc(test1$residuals,"ACF Plot"),plotdensityfunc(test1,"Density Plot"),ncol=2,nrow = 2,heights=c(5,1)),nrow=2)
Box.test(test1$residuals,lag=20, type="Ljung")
...
test1_forecast <- forecast(test1, level=c(95), h=5)

x <- window(Localidade_TS, 365, c(366,24))
y <- test1_forecast$mean
z <- test1_forecast$upper
w <- test1_forecast$lower

ts.plot(x, y, z, w,  gpars = list(col = c("black", "brown1", "brown", "brown")), main = "Time series forecasting plot", ylab = "Values")

round(cbind(as.matrix(test1_forecast$mean),as.matrix(test1_forecast$lower),as.matrix(test1_forecast$upper)),2)
