...
setwd("C:/Users/Fernando/Desktop/ST/Estarreja")
data <- read_excel("C:/Users/Fernando/Desktop/ST/QualidadeARO3.xlsx")
Localidade <- data$Estarreja
...
Box.test(test6$residuals,lag=19, type="Ljung")
...
Box.test(test7$residuals,lag=19, type="Ljung")

#Forecast for best model
test7_forecast <- forecast(test7, level=c(95), h=5)

x <- window(Localidade_TS, 365, c(366,24))
y <- test7_forecast$mean
z <- test7_forecast$upper
w <- test7_forecast$lower

ts.plot(x, y, z, w,  gpars = list(col = c("black", "brown1", "brown", "brown")), main = "Time series forecasting plot", ylab = "Values")

test7$coef

round(cbind(as.matrix(test7_forecast$mean),as.matrix(test7_forecast$lower),as.matrix(test7_forecast$upper)),2)
...
ent_decomposition3<-Arima(Localidade_TS, order=c(2,0,0), seasonal=c(0,1,2), lambda="auto")

ggtsdisplay(ent_decomposition1$residuals, main = "order=c(0,0,0), seasonal=c(0,1,0)")
ggtsdisplay(ent_decomposition2$residuals, main = "order=c(0,0,0), seasonal=c(0,1,1)")
ggtsdisplay(ent_decomposition3$residuals, main = "order=c(0,0,0), seasonal=c(1,1,1)")
ggtsdisplay(ent_decomposition4$residuals, main = "order=c(1,0,0), seasonal=c(1,1,1)")
ggtsdisplay(ent_decomposition5$residuals, main = "order=c(1,0,1), seasonal=c(1,1,1)")
